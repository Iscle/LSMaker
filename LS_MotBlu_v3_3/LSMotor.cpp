...
float fact[33] = {
  0, 0.08538163, 0.112548512, 0.131953428, 0.138421734, 0.144890039, 0.151358344, 0.155239327, 0.160413972,
...
void LSMt::LsMtDreta(float radi,float initVeloc,float initGraus,float veloc,float graus,float endVeloc,float endGraus) {
  if(MT.actiu==0) {
    ...
    if (MT.space>0) {
      ...
      if (MT.Lveloc>=0) MT.Ldir = 1; 
      else MT.Ldir = -1;
      if (MT.Rveloc>=0) MT.Rdir = 1; 
      else MT.Rdir = -1;
    } 
    else {
      if (MT.accel==1) {
        ...
        if (MT.initLveloc>=0) MT.Ldir = 1; 
        else MT.Ldir = -1;
        if (MT.initRveloc>=0) MT.Rdir = 1; 
        else MT.Rdir = -1;
      } 
      else {
        ...
        if (MT.endLveloc>=0) MT.Ldir = 1; 
        else MT.Ldir = -1;
        if (MT.endRveloc>=0) MT.Rdir = 1; 
        else MT.Rdir = -1;
      }
    }
    if (MT.space>0) {
...
void LSMt::LsMtFastAutocalibrate(void) {
  ...
  Serial.print("FOR. "); 
  Serial.print(MaxTicsF); 
  Serial.print(" -> "); 
  Serial.print(PercLF); 
  Serial.print(":"); 
  Serial.println(PercRF);
  digitalWrite(LWenable,LOW);
  digitalWrite(RWenable,LOW);
  delay(500);
  ...
  Serial.print("BAC. "); 
  Serial.print(MaxTicsB); 
  Serial.print(" -> "); 
  Serial.print(PercLB); 
  Serial.print(":"); 
  Serial.println(PercRB);
  digitalWrite(LWenable,LOW);
  digitalWrite(RWenable,LOW);
  delay(500);
}
